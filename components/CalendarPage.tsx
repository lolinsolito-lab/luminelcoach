...
import {
    ...
    MoonIcon, StarIcon, PlusIcon, XMarkIcon, PlayIcon,
    LockClosedIcon, FireIcon, BellIcon,
...
const canAccess = (up: string, ep: string) => PLAN_ORDER[up] >= PLAN_ORDER[ep];

// ── Seed events (finché non c'è Supabase calendar table) ─────────────────────
const makeSeedEvents = () => {
    ...
    return [
        ...
        { id: "course-ml", title: "The Art of Mindful Living · Day 18", type: "course", date: today, time: "20:00", duration: 20, plan: "free", completed: false, booked: true, desc: "Integrazione Profonda.", linked: "courses", progress: 85 },
        { id: "calm-1", title: "Gamma Wave · Focus Estremo", type: "calm", date: today, time: "15:00", duration: 25, plan: "premium", completed: false, booked: true, desc: "Frequenza 40Hz per produttività.", linked: "experiences" },
        { id: "session-1", title: "Sessione AI · Shadow Work", type: "session", date: d(1), time: "10:00", duration: 45, plan: "premium", completed: false, booked: false, desc: "Sessione profonda con Luminel.", linked: "chat" },
        { id: "council-1", title: "Il Consiglio degli Archetipi", type: "session", date: d(1), time: "18:00", duration: 60, plan: "vip", completed: false, booked: false, desc: "Convoca i 4 archetipi per analizzare la tua sfida.", linked: "council" },
        { id: "rq-2", title: "Reality Quest · Lettera a Te Stesso", type: "quest", date: d(2), time: "09:00", duration: 30, plan: "free", completed: false, booked: true, desc: "Scrivi una lettera a te stesso di 3 mesi fa.", linked: "quests" },
        { id: "course-dt", title: "Deep Transformation · Day 23", type: "course", date: d(3), time: "08:00", duration: 30, plan: "vip", completed: false, booked: true, desc: "La Mappa delle Credenze.", linked: "courses", progress: 38 },
    ...
    return (
        <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay }}
            ...
            <div className="pl-4 pr-4 py-3.5 flex items-start gap-3">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: `${et.color}20`, color: et.color }}>
                    ...
                    <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                                ...
                                {event.progress !== undefined && <span className="text-[10px]" style={{ color: et.color }}>{event.progress}% completato</span>}
                            </div>
                        ...
                        <div className="flex flex-col items-end gap-1 flex-shrink-0">
                            ...
                            {event.booked && !event.completed && ok && <span className="text-[9px]" style={{ color: "#4A9ED4" }}>• Prenotato</span>}
...
// ── Event detail modal ────────────────────────────────────────────────────────
const EventModal: React.FC<{ event: any; userPlan: string; onClose: () => void; onToggleBook: (id: string) => void; onComplete: (id: string) => void }> =
    ({ event, userPlan, onClose, onToggleBook, onComplete }) => {
        const et = EVENT_TYPES[event.type];
        const ok = canAccess(userPlan, event.plan);
        const linkedLabels: Record<string, string> = { chat: "Apri Chat AI", courses: "Vai ai Corsi", experiences: "Vai alle Experiences", quests: "Vai alle Quests", council: "Vai al Consiglio", call: "Vai al Voice Coach" };
        return (
                ...
                <motion.div initial={{ opacity: 0, y: 20, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                    ...
                    <div className="p-5 pb-4 relative" style={{ background: `${et.color}10` }}>
                        ...
                        <div className="flex items-start justify-between">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `${et.color}20`, color: et.color }}>{et.icon}</div>
                                <div>
                            ...
                            <button onClick={onClose} className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.05)", color: DL.muted }}>
                    ...
                    <div className="p-5">
                        <div className="grid grid-cols-2 gap-3 mb-4">
                            {[{ label: "Orario", value: event.time }, { label: "Durata", value: `${event.duration} min` }, { label: "Piano", value: event.plan.toUpperCase() }].map((m, i) => (
                        ...
                        {event.progress !== undefined && (
                            <div className="mb-4 px-3 py-2.5 rounded-xl" style={{ background: DL.glass, border: `0.5px solid ${DL.glassB}` }}>
                                <div className="flex justify-between text-[11px] mb-2"><span style={{ color: DL.muted }}>Progresso</span><span style={{ color: et.color }}>{event.progress}%</span></div>
                                <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.05)" }}>
                                    <div className="h-full rounded-full" style={{ width: `${event.progress}%`, background: et.color }} />
                                </div>
                            </div>
                        )}
                        {event.desc && <p className="text-[12px] leading-relaxed mb-5" style={{ color: "rgba(240,235,224,0.65)" }}>{event.desc}</p>}
                        {ok ? (
                            <div className="flex flex-col gap-2">
                                {!event.completed && (
                                    <div className="flex gap-2">
                                        <button onClick={() => onToggleBook(event.id)}
                                            ...
                                            style={event.booked ? { background: "rgba(255,255,255,0.04)", color: DL.muted, border: `0.5px solid ${DL.glassB}` } : { background: `${et.color}18`, color: et.color, border: `0.5px solid ${et.color}35` }}>
                                ...
                                {event.completed && <div className="py-2.5 rounded-xl text-[12px] text-center" style={{ background: "rgba(16,185,129,0.08)", color: "#10B981", border: "0.5px solid rgba(16,185,129,0.25)" }}>✓ Completato</div>}
                                {event.linked && (
                                    <button className="w-full py-2.5 rounded-xl text-[12px] font-medium flex items-center justify-center gap-2"
                                        style={{ background: `${et.color}10`, color: et.color, border: `0.5px solid ${et.color}25` }}>
                                        <PlayIcon className="w-3.5 h-3.5" />{linkedLabels[event.linked] || "Apri"}
                                    </button>
                                )}
                            </div>
                        ) : (
                            <div className="py-4 rounded-xl text-center" style={{ background: "rgba(212,96,58,0.06)", border: "0.5px solid rgba(212,96,58,0.2)" }}>
                                ...
                                <p className="text-[10px] mt-0.5" style={{ color: DL.muted }}>Aggiorna il piano per accedere</p>
                            </div>
                        )}
                    </div>
                </motion.div>
...
// ── New event modal — Dark Luxury ─────────────────────────────────────────────
const NewEventModal: React.FC<{ onClose: () => void; onSave: (d: any) => void }> = ({ onClose, onSave }) => {
    const [data, setData] = useState({ title: "", type: "personal", date: new Date().toISOString().split("T")[0], time: "09:00", duration: 30, desc: "" });
    const et = EVENT_TYPES[data.type];
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            style={{ background: "rgba(6,6,15,0.9)", backdropFilter: "blur(16px)" }}
            onClick={e => e.target === e.currentTarget && onClose()}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
                ...
                style={{ background: DL.surface, border: `0.5px solid ${et.color}35` }}>
                {/* Header */}
                <div className="p-5 relative" style={{ background: `${et.color}09` }}>
                    <div className="absolute top-0 left-0 right-0 h-px" style={{ background: `linear-gradient(90deg,transparent,${et.color}55,transparent)` }} />
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: `${et.color}18`, color: et.color }}>{et.icon}</div>
                            <div>
                                <div className="text-[9px] tracking-[0.16em] uppercase" style={{ color: et.color }}>Nuovo Evento</div>
                                <div className="text-[15px] font-medium" style={{ color: DL.white, fontFamily: "'Cormorant Garamond',serif" }}>Aggiungi al Journey</div>
                            </div>
                        </div>
                        <button onClick={onClose} className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.05)", color: DL.muted }}>
                            <XMarkIcon className="w-4 h-4" />
                ...
                <div className="p-5 flex flex-col gap-4">
                    {/* Titolo */}
                    <div>
                        <div className="text-[10px] tracking-[0.14em] uppercase mb-2" style={{ color: DL.muted }}>Titolo</div>
                        <input value={data.title} onChange={e => setData({ ...data, title: e.target.value })}
                            placeholder="Es. Journaling Ikigai, Meditazione..."
                            style={{ width: "100%", padding: "10px 14px", background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.08)", borderRadius: 9, fontSize: 13, color: DL.white, outline: "none", fontFamily: "'DM Sans',sans-serif", boxSizing: "border-box" }}
                            onFocus={e => e.target.style.borderColor = "rgba(201,168,76,0.4)"}
                            onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.08)"} />
                    </div>
                    {/* Tipo */}
                    <div>
                        <div className="text-[10px] tracking-[0.14em] uppercase mb-2" style={{ color: DL.muted }}>Tipo</div>
                        <div className="grid grid-cols-3 gap-1.5">
                            {Object.entries(EVENT_TYPES).map(([type, cfg]) => (
                                <button key={type} onClick={() => setData({ ...data, type })}
                                    className="py-2 px-2 rounded-lg text-[10px] text-center transition-all border flex items-center justify-center gap-1"
                                    style={data.type === type ? { background: `${cfg.color}18`, borderColor: `${cfg.color}45`, color: cfg.color } : { background: "rgba(255,255,255,0.02)", borderColor: DL.glassB, color: DL.muted }}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {[{ label: "Data", key: "date", type: "date" }, { label: "Ora", key: "time", type: "time" }].map(f => (
                            <div key={f.key}>
                                <div className="text-[10px] tracking-[0.14em] uppercase mb-2" style={{ color: DL.muted }}>{f.label}</div>
                                <input type={f.type} value={(data as any)[f.key]} onChange={e => setData({ ...data, [f.key]: e.target.value })}
                                    style={{ width: "100%", padding: "9px 12px", background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.08)", borderRadius: 9, fontSize: 12, color: DL.white, outline: "none", fontFamily: "'DM Sans',sans-serif", boxSizing: "border-box", colorScheme: "dark" }}
                                    onFocus={e => e.target.style.borderColor = "rgba(201,168,76,0.4)"}
                                    onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.08)"} />
                            </div>
                        ))}
                    ...
                    {/* Durata */}
                    <div>
                        <div className="text-[10px] tracking-[0.14em] uppercase mb-2" style={{ color: DL.muted }}>Durata</div>
                        <div className="flex gap-2">
                            {[10, 20, 30, 45, 60].map(min => (
                                <button key={min} onClick={() => setData({ ...data, duration: min })}
                                    className="flex-1 py-2 rounded-lg text-[11px] transition-all border"
                                    style={data.duration === min ? { background: DL.goldDim, borderColor: DL.goldB, color: DL.gold } : { background: "rgba(255,255,255,0.02)", borderColor: DL.glassB, color: DL.muted }}>
                                    {min}m
                                </button>
                            ))}
                        </div>
                    ...
                    {/* Note */}
                    <div>
                        <div className="text-[10px] tracking-[0.14em] uppercase mb-2" style={{ color: DL.muted }}>Note (opzionale)</div>
                        <textarea value={data.desc} onChange={e => setData({ ...data, desc: e.target.value })}
                            placeholder="Aggiungi una nota..."
                            rows={2}
                            style={{ width: "100%", padding: "10px 14px", background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.08)", borderRadius: 9, fontSize: 12, color: DL.white, outline: "none", fontFamily: "'DM Sans',sans-serif", resize: "none", boxSizing: "border-box" }}
                            onFocus={e => e.target.style.borderColor = "rgba(201,168,76,0.4)"}
                            onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.08)"} />
                    </div>
                    {/* Actions */}
                    <div className="flex gap-2 pt-1">
                        <button onClick={onClose}
                            style={{ flex: 1, padding: "11px", borderRadius: 9, background: "rgba(255,255,255,0.03)", border: `0.5px solid ${DL.glassB}`, color: DL.muted, fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>
                            Annulla
                        </button>
                        <button onClick={() => { if (!data.title.trim()) return; onSave({ ...data, date: new Date(data.date + "T" + data.time) }); }}
                            disabled={!data.title.trim()}
                            style={{ flex: 1, padding: "11px", borderRadius: 9, background: data.title.trim() ? DL.gold : "rgba(201,168,76,0.2)", color: data.title.trim() ? "#06060F" : "rgba(201,168,76,0.4)", border: "none", fontSize: 12, fontWeight: 600, cursor: data.title.trim() ? "pointer" : "not-allowed", fontFamily: "'DM Sans',sans-serif" }}>
                            Salva Evento
                        </button>
                    </div>
                </div>
...
const CalendarPage: React.FC = () => {
    ...
    const [events, setEvents] = useState<any[]>(makeSeedEvents);
    ...
    const formatTitle = () => {
        ...
        if (viewMode === "week") {
            const w = getWeekDays(), s = w[0].date, end = w[6].date;
    ...
    const completedToday = todayEvs.filter(e => e.completed).length;
    const bookedToday = todayEvs.filter(e => e.booked && !e.completed).length;
    const weekAll = getWeekDays().flatMap(d => d.events);
    const weekDone = weekAll.filter(e => e.completed).length;

    return (
        <div className="relative min-h-screen pb-24 max-w-full mx-auto" style={{ background: "#06060F" }}>
            {/* Ambient */}
            <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
                <div className="absolute -top-20 -right-20 w-96 h-96 rounded-full" style={{ background: "rgba(201,168,76,0.04)", filter: "blur(100px)" }} />
                <div className="absolute -bottom-10 -left-10 w-80 h-80 rounded-full" style={{ background: "rgba(155,116,224,0.05)", filter: "blur(100px)" }} />
            </div>

            {/* ── STICKY HEADER ── */}
            <div className="sticky top-0 z-20" style={{ background: "rgba(9,9,26,0.9)", borderBottom: `0.5px solid ${DL.dim}`, backdropFilter: "blur(20px)" }}>
                <div className="px-5 py-4 max-w-7xl mx-auto">
                    ...
                    <div className="flex gap-2 flex-wrap mb-4">
                        <MiniStat icon={<FireSolid className="w-3.5 h-3.5" />} label="Oggi" value={`${completedToday}/${todayEvs.length} eventi`} color={DL.guer} />
                    ...
                    <div className="flex gap-2 mb-4">
                        {(["calendar", "tracking"] as const).map(tab => (
                            <button key={tab} onClick={() => setActiveTab(tab)}
                                ...
                                style={activeTab === tab ? { background: DL.goldDim, borderColor: DL.goldB, color: DL.goldBr } : { background: "rgba(255,255,255,0.02)", borderColor: DL.glassB, color: DL.muted }}>
                            ...
                            <div className="flex rounded-xl overflow-hidden flex-shrink-0" style={{ border: `0.5px solid ${DL.glassB}` }}>
                                {(["day", "week", "month"] as const).map(m => (
                                    <button key={m} onClick={() => setViewMode(m)}
                                        ...
                                        style={viewMode === m ? { background: DL.goldDim, color: DL.gold } : { background: "transparent", color: DL.muted }}>
                                        {m === "day" ? "Giorno" : m === "week" ? "Settimana" : "Mese"}
                                    </button>
                                ))}
                            </div>
                            <div className="flex items-center gap-3">
                                <button onClick={() => navDate(-1)} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: DL.glass, border: `0.5px solid ${DL.glassB}`, color: DL.muted }}>
                                ...
                                {Object.entries(EVENT_TYPES).map(([type, cfg]) => {
                                    ...
                                    return (
                                        <button key={type} onClick={() => setActiveFilters(f => on ? f.filter(x => x !== type) : [...f, type])}
                                            ...
                                            style={on ? { background: `${cfg.color}14`, borderColor: `${cfg.color}40`, color: cfg.color } : { background: "rgba(255,255,255,0.02)", borderColor: DL.glassB, color: DL.muted }}>
                                            {cfg.icon}{cfg.label}
                                        </button>
                                    );
                                })}
                                    ...
                                    <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                                        <div className="flex items-center gap-3 mb-4">
                                            <div className="w-10 h-10 rounded-xl flex flex-col items-center justify-center flex-shrink-0"
                                                style={isToday ? { background: DL.goldDim, border: `0.5px solid ${DL.goldB}` } : { background: DL.glass, border: `0.5px solid ${DL.glassB}` }}>
                                                <div className="text-[9px]" style={{ color: isToday ? DL.gold : DL.muted }}>{day.date.toLocaleDateString("it-IT", { weekday: "short" }).toUpperCase()}</div>
                                                <div className="text-[15px] font-medium leading-tight" style={{ color: isToday ? DL.goldBr : DL.white }}>{day.date.getDate()}</div>
                                            </div>
                                            <div>
                                                ...
                                                <div className="text-[11px]" style={{ color: DL.muted }}>
                                                    ...
                                                    {day.events.filter(e => e.completed).length > 0 && <span style={{ color: "#10B981" }}> · {day.events.filter(e => e.completed).length} completati</span>}
                                                </div>
                                            </div>
                                        </div>
                                        {day.events.length > 0 ? (
                                            <div className="space-y-2" style={{ marginLeft: "52px" }}>
                                                {day.events.sort((a, b) => a.time.localeCompare(b.time)).map((ev, j) => (
                                                    <EventCard key={ev.id} event={ev} userPlan={userPlan} onSelect={setSelectedEvent} delay={i * 0.05 + j * 0.03} />
                                                ))}
                                            </div>
                                        ) : (
                                            <div className="text-center py-8 rounded-xl" style={{ marginLeft: "52px", background: DL.glass, border: `0.5px solid ${DL.glassB}` }}>
                                ...
                                {getDaysInMonth(currentDate).map((day, i) => {
                                    ...
                                    return (
                                        <motion.div key={i} whileHover={{ scale: 1.02, zIndex: 10 }} whileTap={{ scale: 0.98 }}
                                            ...
                                            style={{ borderBottom: `0.5px solid ${DL.dim}`, borderRight: `0.5px solid ${DL.dim}`, background: isSel ? DL.goldDim : isToday ? "rgba(201,168,76,0.04)" : "transparent", opacity: day.isCurrentMonth ? 1 : 0.4 }}>
                                            ...
                                            <div className="space-y-0.5">
                                                ...
                                                {dayEvs.length > 2 && <div className="text-[9px] pl-1" style={{ color: DL.muted }}>+{dayEvs.length - 2}</div>}
                                            </div>
                                        </motion.div>
                                    );
                                })}
                    ...
                    {viewMode === "day" && (
                        <div>
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <div className="font-serif text-[22px] font-normal capitalize"
                                        style={{ color: selectedDate.toDateString() === new Date().toDateString() ? DL.gold : DL.white }}>
                                        {selectedDate.toLocaleDateString("it-IT", { weekday: "long", day: "numeric", month: "long" })}
                                        {selectedDate.toDateString() === new Date().toDateString() && <span className="ml-2 text-[14px] opacity-70">· Oggi</span>}
                                    </div>
                                    <div className="text-[12px]" style={{ color: DL.muted }}>{getEventsForDate(selectedDate).length} eventi</div>
                                </div>
                            </div>
                            {getEventsForDate(selectedDate).length > 0 ? (
                                    ...
                                    <div className="space-y-4">
                                        {getEventsForDate(selectedDate).sort((a, b) => a.time.localeCompare(b.time)).map((ev, i) => (
                                            <motion.div key={ev.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                                                ...
                                                <div className="w-14 flex-shrink-0 text-right pt-3"><span className="text-[11px]" style={{ color: DL.muted }}>{ev.time}</span></div>
                                                ...
                                                <div className="flex-1"><EventCard event={ev} userPlan={userPlan} onSelect={setSelectedEvent} /></div>
                                            </motion.div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="text-center py-16 rounded-xl" style={{ background: DL.glass, border: `0.5px solid ${DL.glassB}` }}>
                                    ...
                                    <p className="text-[12px]" style={{ color: DL.muted }}>Nessun evento · Goditi il riposo ✨</p>
                                    <button onClick={() => setShowNewEventModal(true)}
            ...
            <AnimatePresence>
                {showNewEventModal && (
                    <NewEventModal onClose={() => setShowNewEventModal(false)}
                        onSave={(d: any) => {
                            setEvents(ev => [...ev, { id: `custom-${Date.now()}`, ...d, plan: userPlan, completed: false, booked: true }]);
